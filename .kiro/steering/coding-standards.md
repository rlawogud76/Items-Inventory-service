...
### 2025-01-25: 배점 설정 메시지 즉시 사라지는 버그 수정 (완료)
- **문제**: 재고/제작 임베드에서 "⭐ 배점 설정" 버튼 클릭 시 메시지가 즉시 사라짐
- **원인 1**: `interaction.update()`로 원본 임베드 메시지를 덮어쓴 후 자동 삭제 타이머 실행
- **원인 2**: 여러 타이머가 동시에 실행되어 충돌 (이전 타이머가 취소되지 않음)
- **해결**: 
  - 첫 클릭 시 `interaction.reply({ ephemeral: true })`로 새 임시 메시지 생성
  - 뒤로가기 버튼 클릭 시 `interaction.update()`로 기존 메시지 업데이트
  - `isBackButton` 파라미터로 두 경우 구분
  - **타이머 관리 시스템 추가**: `activeTimers` Map으로 messageId별 타이머 추적
  - 새 타이머 등록 시 이전 타이머 자동 취소 (`clearTimeout`)
  - Unknown Message 에러(10008) 무시 처리
- **수정된 파일**: points.js, buttons.js
- **핵심 교훈**:
  - 임베드 메시지의 버튼은 원본 메시지를 보존해야 함
  - 설정 메뉴는 새 ephemeral 메시지로 열어야 함
  - `interaction.update()`는 원본 메시지를 덮어쓰므로 주의
  - **여러 단계의 메뉴에서는 타이머 관리가 필수** (이전 타이머 취소)
  - `getTimeoutSettings()`는 async가 아니므로 직접 DB에서 로드해야 함
- **커밋**: 
  - "Fix: 배점 설정 버튼 클릭 시 메시지 즉시 사라지는 버그 수정 (update → reply ephemeral)"
  - "Debug: 배점 관리 버튼 핸들러에 디버그 로그 추가"
  - "Fix: 배점 관리 타이머 설정 로드 방식 수정 및 디버그 로그 추가"
  - "Fix: 배점 설정 타이머 충돌 해결 (이전 타이머 자동 취소)"

### 2025-01-25: 모든 페이지네이션에 페이지 점프 기능 추가 (완료)
- **새 기능**: 페이지 번호 버튼 클릭 시 모달로 원하는 페이지로 직접 이동
- **적용 범위**: 
  - `/재고`, `/제작` 임베드 메시지 (완전 지원)
  - 수량 관리 ephemeral 메시지 (완전 지원)
  - 기타 페이지네이션 (버튼 활성화, 미구현 안내 메시지 표시)
- **수정된 파일**: pagination.js, paginationUtils.js, buttons.js, modals.js, buttonHandlers/index.js
- **추가된 핸들러**:
...
- **적용 내용**:
  ...
  - 입력한 페이지로 즉시 이동
  - 페이지 번호 검증 (1~총페이지 범위)
  - `interaction.update()` 사용으로 ephemeral 메시지에서도 작동
  - 수량 관리 페이지 점프 완전 구현
  - 기타 페이지네이션은 버튼만 활성화 (클릭 시 안내 메시지)
- **커밋**:
  - "Add: 범용 페이지 점프 기능 추가 (모든 페이지네이션에 적용)"
  - "Fix: 범용 페이지 점프 customId 파싱 로직 수정"
  - "Fix: handleGenericPageJump export 누락 수정"
  - "Fix: 페이지 점프 기능을 임베드 페이지네이션만 지원하도록 수정 (interaction.reply 에러 해결)"
  - "Docs: 페이지 점프 기능 작업 이력 업데이트 (임베드만 지원)"
  - "Add: 모든 페이지네이션에 페이지 점프 기능 재활성화 (수량 관리 지원)"
