...
function createCraftingEmbed(crafting, categoryName = null, uiMode = 'normal', barLength = 10) {
  ...
  if (categoryName) {
    ...
    if (!crafting.categories[categoryName] || Object.keys(crafting.categories[categoryName]).length === 0) {
      embed.setDescription('?? 등록된 제작품이 없습니다.');
      return embed;
    }
    ...
    if (!crafting.categories || Object.keys(crafting.categories).length === 0) {
      embed.setDescription('?? 등록된 카테고리가 없습니다.');
      return embed;
    }
  ...
  const row2 = new ActionRowBuilder()
    .addComponents(
      new ButtonBuilder()
        ...
        .setLabel(autoRefresh ? '?? 자동새로고침 중지' : '▶? 자동새로고침')
  ...
  if (interaction.isCommand()) {
    ...
    try {
      if (commandName === '재고') {
        ...
        console.log(`▶? 자동 새로고침 시작: ${messageId} (재고 - ${category})`);
      }
        ...
        autoRefreshTimers.set(messageId, refreshInterval);
        console.log(`▶? 자동 새로고침 시작: ${messageId} (제작 - ${category})`);
      }
  ...
  if (interaction.isButton()) {
    ...
    if (interaction.customId.startsWith('refresh')) {
      ...
      try {
        ...
        await interaction.update({
          content: `??? **${category}** 카테고리에서 삭제할 ${type === 'inventory' ? '물품' : '품목'}을 선택하세요:`,
          components: [row]
        });
      ...
      try {
        ...
        const itemOptions = items.map(item => {
          ...
          if (percentage >= 100) {
            ...
            description = `?? ${workingUser.userName}님이 ${isCrafting ? '제작' : '수집'} 중입니다`;
          }
  ...
  if (interaction.isStringSelectMenu()) {
    if (interaction.customId.startsWith('select_recipe_edit_')) {
      ...
      try {
        ...
        if (oldQuantity === 0) {
          return await interaction.update({
            content: `?? **${selectedItem}**은(는) 이미 0개입니다.`,
            components: []
          });
        }
