...
function StatCard({ title, value, icon: Icon, color, to }) {
  const content = (
    <div className={`bg-dark-300 rounded-xl p-6 border border-dark-100 hover:border-${color}-500/50 transition-colors`}>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-dark-300 rounded-xl p-6 border border-dark-100">
        ...
        {contributionsData?.contributors?.length > 0 ? (
              ...
              >
                <div className="flex items-center gap-3">
                  <span className={`
                    ...
                    ${index > 2 ? 'bg-dark-100 text-gray-500' : ''}
                  `}>
                    {index + 1}
                  </span>
          ...
          <p className="text-gray-400 text-center py-8">아직 기여도 데이터가 없습니다.</p>
        )}
      </div>
    </div>
  )
